...
// TODO : has to get the userVotedYetState somehow
...
export class VotingFormElement extends LitElement {
  ...
  private updateYesVotes() {
    if (!this.hasUserVotedYet) {
      this.numberYes += 1;
      this.hasUserVotedYet = true;
  ...
  private checkMajorityDecision() {
    ...
    } else if (this.numberNo >= majority) {
